...
st.pyplot(plt)
st.write("Стоит отметить значительный дизбаланс в распределении целевой переменной: из 15 тысяч объектов - 13 тысяч клиентов не откликнулись на предложение банка, и только чуть менее двух тысяч человек сделали отклик на услугу банка. Такое распределение в дальнейшем повлияет и на предсказательную модель.")
st.markdown("---")
